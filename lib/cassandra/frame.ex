defmodule Cassandra.Frame do
  ...
  def byte({struct, data}, field) do
    <<n::integer-8, data::binary>> = data
    {struct!(struct, [{field, n}]), data}
  end

  def short({struct, data}, field) do
    <<n::integer-16, data::binary>> = data
    {struct!(struct, [{field, n}]), data}
  end

  def int({struct, data}, field) do
    <<n::integer-32, data::binary>> = data
    {struct!(struct, [{field, n}]), data}
  end

  def string({struct, data}, field) do
    <<n::integer-16, data::binary>> = data
    <<s::binary-size(n), data::binary>> = data
    {struct!(struct, [{field, s}]), data}
  end

  def bytes({struct, data}, field) do
    <<n::integer-32, data::binary>> = data
    <<s::binary-size(n), data::binary>> = data
    {struct!(struct, [{field, s}]), data}
  end
  ...
  def read_byte(data) do
    <<n::integer-signed-8, data::binary>> = data
    {n, data}
  end
  ...
  def from_binary(header, data) do
    ...
    frame = %{frame | flags: header.flags, stream: header.stream}
